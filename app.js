...
const elements = {
  ...
  librarySearchArea: document.querySelector("#librarySearchArea"),
  textSearchSection: document.querySelector("#textSearchSection"),
  textSearchBookRow: document.querySelector("#textSearchBookRow"),
  textSearchEmpty: document.querySelector("#textSearchEmpty"),
  textSearchResults: document.querySelector("#textSearchResults"),
...
function renderBookGrid() {
  ...
  elements.bookGrid.classList.toggle("search-row", isSearching);

  if (isSearching) {
    elements.librarySectionLabel.textContent = "책 검색";
...
// Whole-Bible verse text, keyed by book code, for 본문검색 (full-text verse
// search) — {"GEN": [ [{v,t}, ...] /* chapter 1 */, [...] /* chapter 2 */, ... ], ...},
// generated by tools/build_search_index.js. Fetched once, lazily, on first
// use rather than up front, since it covers all 1,189 chapters (~5MB).
let verseSearchIndexPromise = null;

function loadVerseSearchIndex() {
  if (!verseSearchIndexPromise) {
    verseSearchIndexPromise = fetch("data/bible-search-index.json").then((res) => {
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return res.json();
    });
  }
  return verseSearchIndexPromise;
}

function bookMatchesTextQuery(index, code, query) {
  const chapters = index[code];
  if (!chapters) return false;
  return chapters.some((verses) => verses.some((verse) => verse.t.toLowerCase().includes(query)));
}

function renderTextSearch() {
  const rawQuery = (state.bookSearchQuery || "").trim();
  ...
  if (state.textSearchOpenBook) {
    elements.librarySearchArea.hidden = true;
    elements.textSearchResults.hidden = false;
    renderTextSearchDetail(state.textSearchOpenBook, rawQuery, query);
    return;
  }

  elements.librarySearchArea.hidden = false;
  elements.textSearchResults.hidden = true;
  elements.textSearchSection.hidden = false;

  const loading = document.createElement("p");
  loading.className = "empty-state";
  loading.textContent = "본문 검색 중...";
  elements.textSearchBookRow.replaceChildren(loading);
  elements.textSearchEmpty.hidden = true;

  loadVerseSearchIndex()
    .then((index) => {
      // The query (or the search itself) may have changed while this was in flight.
      if ((state.bookSearchQuery || "").trim().toLowerCase() !== query || state.textSearchOpenBook) return;

      const matchingBooks = DATA.books.filter((book) => {
        const code = getBookCode(chaptersByBook[book.name][0]);
        return code && bookMatchesTextQuery(index, code, query);
      });

      elements.textSearchBookRow.replaceChildren(
        ...matchingBooks.map((book) => {
          const btn = document.createElement("button");
          btn.type = "button";
          btn.className = "mini-button text-search-book-btn";
      ...
      elements.textSearchEmpty.textContent = "본문에서 찾을 수 없습니다.";
      elements.textSearchEmpty.hidden = matchingBooks.length > 0;
    })
    .catch(() => {
      if ((state.bookSearchQuery || "").trim().toLowerCase() !== query || state.textSearchOpenBook) return;
      elements.textSearchBookRow.replaceChildren();
      elements.textSearchEmpty.textContent = "본문을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.";
      elements.textSearchEmpty.hidden = false;
    });
}

function renderTextSearchDetail(bookName, rawQuery, query) {
  elements.textSearchResultsTitle.textContent = `${bookName} "${rawQuery}" 결과`;

  const loading = document.createElement("p");
  loading.className = "reading-loading";
  loading.textContent = "불러오는 중...";
  elements.textSearchResultsBody.replaceChildren(loading);

  loadVerseSearchIndex()
    .then((index) => {
      if (state.textSearchOpenBook !== bookName) return;
      if ((state.bookSearchQuery || "").trim().toLowerCase() !== query) return;

      const code = getBookCode(chaptersByBook[bookName][0]);
      const chapters = (code && index[code]) || [];
      const nodes = [];
      ...
      chapters.forEach((verses, chapterIdx) => {
        ...
        const flushRun = () => {
          ...
          heading.className = "text-search-group-heading";
          const range = run.length === 1 ? `${run[0].v}절` : `${run[0].v}~${run[run.length - 1].v}절`;
          heading.textContent = `${chapterNum}장 : ${range}`;
          nodes.push(heading);
          run.forEach((verse) => {
        ...
        verses.forEach((verse) => {
          const matches = verse.t.toLowerCase().includes(query);
          if (matches && (!run.length || verse.v === run[run.length - 1].v + 1)) {
            run.push(verse);
          } else {
            flushRun();
            if (matches) run.push(verse);
          }
        });
        flushRun();
      });
    ...
    .catch(() => {
      ...
      failed.className = "reading-loading";
...
// Every .reading-verse whose verse number is in verseSet, in document order
// (which is already reading order) — the shared source of truth for both the
// live preview and the final copy payload.
function getSelectedVerseLines(verseSet) {
  return [...elements.readingText.querySelectorAll(".reading-verse")].filter((el) =>
    verseSet.has(Number(el.dataset.verse))
  );
}

function applyVerseSelection(verseSet) {
  state.verseSelection = verseSet;
  const lines = getSelectedVerseLines(verseSet);
  elements.readingText
    .querySelectorAll(".reading-verse.verse-selected")
    .forEach((el) => el.classList.remove("verse-selected"));
...
// Each tap toggles that verse in or out of the selection, regardless of
// whether it's adjacent to anything already selected — so gapped selections
// like {2, 5, 9} work. Untoggling the last selected verse clears the bar.
function selectVerseForCopy(line) {
  const verseNum = Number(line.dataset.verse);
  const next = new Set(state.verseSelection);
  if (!next.delete(verseNum)) next.add(verseNum);
...
function formatVerseRanges(sortedVerses) {
  const parts = [];
...
async function copySelectedVerse() {
  ...
  if (!sel) return;
  const chapter = getCurrentChapter();
  const lines = getSelectedVerseLines(sel);
  const isMulti = lines.length > 1;
  const body = isMulti
    ? lines.map((el) => `${el.dataset.verse} ${el.dataset.text || ""}`).join("\n")
    : lines[0]?.dataset.text || "";
  const sortedVerses = lines.map((el) => Number(el.dataset.verse));
  const reference = `${chapter.book} ${chapter.chapter}:${formatVerseRanges(sortedVerses)}`;
  const payload = `${body}\n${reference}`;
